...
use crate::theme::{ColorScheme, MaterialTheme};
...
pub fn date_picker_view(
    ...
    let offset: usize = if first_weekday == 0 {
        6
    } else {
        first_weekday - 1
    } as usize;
    ...
    for row in 0..num_rows {
        ...
        all_rows.push(
            Box::new(view::flex(Axis::Horizontal, (day_cells,)).gap(Length::px(2.0)))
                as Box<AnyWidgetView<AppStateNativo>>,
        );
    }
...
pub fn time_picker_view(
    ...
    let _fmt_hora = |h: u32, m: u32| -> String { format!("{:02}:{:02}", h, m) };
    ...
    // Label hora
    let hour_label = view::label(hour_str)
        .text_size(24.0)
        .weight(FontWeight::BOLD)
        .color(on_surface);
    ...
    // Label minuto
    let min_label = view::label(min_str)
        .text_size(24.0)
        .weight(FontWeight::BOLD)
        .color(on_surface);
...
pub fn color_picker_view(
    ...
    let hex_input = view::text_input(
        ...
        move |data: &mut AppStateNativo, new_val: String| {
            let clean = if new_val.starts_with('#') {
                new_val
            } else {
                format!("#{}", new_val)
            };
    ...
    for row in 0..4 {
        ...
        grid_rows.push(
            Box::new(view::flex(Axis::Horizontal, (row_widgets,)).gap(Length::px(4.0)))
                as Box<AnyWidgetView<AppStateNativo>>,
        );
    }

    // Título
    let title = view::label("Selector de Color")
        .text_size(14.0)
        .weight(FontWeight::BOLD)
        .color(on_surface);
